...
// 如果找得到i相同的元素，用layoutItem替换layout数组中的元素
 ...
 */
// 判断两个LayoutItem是否发生碰撞，自己跟自己不算碰撞，紧贴着也不算碰撞，有交错才是碰撞
 ...
 *
 * Modifies item.
 * 根据方向压缩LayoutItem的间隙，比如压缩上方的间隙或压缩左边的间隙
 */
 ...
 * 解决layout里面各个LayoutItem的碰撞和溢出边界的问题
 * @param  {Array} layout Layout array.
...
export function correctBounds(
  ...
  // 找出所有static属性为true的layoutItem，以数组形式返回
  const collidesWith = getStatics(layout);
  for (let i = 0, len = layout.length; i < len; i++) {
    const l = layout[i];
    // Overflows right
    // 如果一个layoutItem溢出到右边界外面去了，那么移动它的x使它右边紧贴右边界
    if (l.x + l.w > bounds.cols) l.x = bounds.cols - l.w;
    // Overflows left
    // 如果一个layoutItem溢出到左边界外面去了，那么移动它的x使它左边紧贴左边界
    if (l.x < 0) {
      l.x = 0;
      // 宽度沾满吗？存疑
      l.w = bounds.cols;
    }
    // 如果是非static的layoutItem，加入数组里去
 ...
 * 在layout中找到第一个与目标LayoutItem发生碰撞的LayoutItem
 ...
 * 对LayoutItem按y优先从小到大排序，如果y相同，再按x从小到大排序
 */
 ...
 * 对LayoutItem数组按x优先排序，从小到大排序，如果x相同再按y从小到大排序
 * Does not modify Layout.
 */
...
export function synchronizeLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>, i: number) => {
    ...
    // 用layoutItem.i与child.key比较，child是否早已存在
    const exists = getLayoutItem(initialLayout, String(child.key));
    if (exists) {
      // 如果存在，深克隆这个layoutItem
      layout[i] = cloneLayoutItem(exists);
    } else { // 如果不存在，要么从child里克隆，要么重新生成一个放在容器最后的layoutItem
      // 非生产环境，提示用户从React15.2开始_grid属性已被废弃
      ...
      // 获取child对应的data-grid属性,这是个layoutItem对象、描述自己
